...
async function checkBalance() {
  try {
    ...
    const wallet = await WalletBuilder.buildFromSeed(
      ...
      networkConfig.node,
      "reveal pole eyebrow convince across impulse bar hundred charge sphere immense make inquiry broken problem menu wine ill soccer donkey insect web picnic leisure",
      getZswapNetworkId(),
      "info"
    );
    ...
    if (balance === 0n) {
      ...
      console.log(
        chalk.white("   1. ") +
          chalk.cyan("Visit: ") +
          chalk.underline("https://midnight.network/test-faucet")
      );
      console.log(
        chalk.white("   2. ") +
          chalk.cyan("Paste your wallet address (shown above)")
      );
      console.log(
        chalk.white("   3. ") + chalk.cyan("Request tokens from the faucet")
      );
      console.log(
        chalk.white("   4. ") + chalk.cyan("Wait 2-5 minutes for processing")
      );
      console.log(
        chalk.white("   5. ") +
          chalk.cyan("Run ") +
      ...
      console.log(
        chalk.gray(
          "💡 Tip: Faucet transactions typically take 2-5 minutes to process."
        )
      );
      console.log(chalk.gray("━".repeat(60)));
    } else {
